...
function createTypedI18n() {
  ...
  const headerI18nContent = ref<HeaderContent>(
    updateHeaderI18nContent(messages.value[locale.value]),
  );
  const homePageI18nContent = ref<HomePageContent>(
    updateHomePageI18nContent(messages.value[locale.value]),
  );
  const aboutMePageI18nContent = ref<AboutMePageContent>(
    updateAboutMePageI18nContent(messages.value[locale.value]),
  );
  const skillsPageI18nContent = ref<SkillsPageContent>(
    updateSkillsPageContent(messages.value[locale.value]),
  );

  const changeLanguage = (lang: 'it' | 'en'): void => {
    locale.value = lang;
  };
