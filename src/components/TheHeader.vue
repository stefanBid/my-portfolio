<script setup lang="ts">
    ...
    const headerLogo = ref<HeaderItem>({name:'home', label:'My Portfolio.'});
    ...
    const change = function(isLogo = true){
        if(!isLogo){
...
<template>
    <header class="header">
        <RouterLink 
            :to="{name:headerLogo?.name}" 
            class="logo" 
            @click="change(false)"
        >
            <i class="bx bxs-terminal" /> {{ headerLogo?.label }}
        </RouterLink>
        <i 
            id="menu-icon" 
            :class="hamburgerIcon" 
